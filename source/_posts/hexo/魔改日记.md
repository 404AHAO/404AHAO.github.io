---
title: anzhiyu主题的魔改日记
abbrlink: 316f
date: 2023-07-24 04:27:34
tags:
---

> 本篇为博主的部分魔改安知鱼主题的重装日记，以防自己日后因魔改迷失所做记录 📝
- 大部分方法通过外部引入，无需修改主题源文件
- 文件路径为了便于管理会以引入文件命名文件夹
- [部分引用魔改](https://www.fomal.cc/posts/d1927166.html)
# FPS设置

新建文件`source/custoem/fps/fps.css`或在已引入的`css`中新增以下内容
...
在根目录 `source` 下新建文件`source/custoem/fps/fps.js`或在已引入的`js`中新增以下内容
```js
...
最后在`_config.anzhiyu.yml`主题配置文件下`inject`配置项中`bottom`处进行引用
...
该方法由`Kouseki`提供：[原文访问链接](https://blog.kouseki.cn/posts/dda6.html)

## 操作步骤
1、新增`css`内容
    - 新建文件`source/custoem/home_link/home.css`或在已引入的`css`中新增以下内容
...
inject:
  head:
    # 自定义css
    - <link rel="stylesheet" href="/custom/home_link/home.css"> # 首页文章卡片擦亮效果
```
...
新建文件`source/custoem/color/color.css`或在已引入的`css`中新增以下内容
...
inject:
  head:
    - <link rel="stylesheet" href="/custom/color/color.css"> # 主题背景色
```

# 关于页面悬浮小标签

新建文件`source/custoem/about-tag/about-tag.css`或在已引入的`css`中新增以下内容
...
inject:
  head:
    - <link rel="stylesheet" href="/custom/about-tag/about-tag.css"> # 关于页悬浮小标签
```

# 公祭日和节日弹框

新建文件`source/custom/dayalert/day.js`或在已引入的`js`中新增以下内容
```js
var d = new Date();
m = d.getMonth() + 1;
dd = d.getDate();
...
新建文件`source/custom/dayalert/lunar.js`或在已引入的`js`中新增以下内容
```js
...
新建文件`source/custom/dayalert/sweetalert2.all.js`或在已引入的`js`中新增以下内容
```js
/*!
* sweetalert2 v8.19.0
...
在`_config.anzhiyu.yml`主题配置文件下`inject`配置项中`bottom`处
引入上面的三个`js`文件
...
inject:
  bottom:
    # 公祭日和节日弹框
    - <script defer type="text/javascript" src="/custom/dayalert/sweetalert2.all.js"></script> # 节日弹窗依赖
    - <script defer src="/custom/dayalert/lunar.js"></script> # 农历计算
    - <script defer src="/custom/dayalert/day.js"></script> # 节日弹窗
